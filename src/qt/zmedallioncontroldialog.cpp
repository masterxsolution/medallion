...
#include "ui_zmedallioncontroldialog.h"
...
std::list<std::string> ZMedallionControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMedallionControlDialog::listMints;

ZMedallionControlDialog::ZMedallionControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMedallionControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMedallionControlDialog::~ZMedallionControlDialog()
{
    delete ui;
}

void ZMedallionControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMedallionControlDialog::updateList()
{
...
void ZMedallionControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMedallionControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMedallion_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMedallionControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMedallionControlDialog::GetSelectedMints()
{
...
void ZMedallionControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
