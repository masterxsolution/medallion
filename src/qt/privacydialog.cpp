...
#include "zmedallioncontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMDL ought to be enough for anybody." - Bill Gates, 2017
    ui->zMDLpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMDLSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMDL->setEnabled(false);
        ui->pushButtonMintzMDL->setToolTip(tr("zMDL is currently disabled due to maintenance."));

        ui->pushButtonSpendzMDL->setEnabled(false);
        ui->pushButtonSpendzMDL->setToolTip(tr("zMDL is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMDLpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMDL_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMDL is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMDL...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMDL in ") + 
...
void PrivacyDialog::on_pushButtonSpendzMDL_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMDL is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMDL
        sendzMDL();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMDL
    sendzMDL();
}

void PrivacyDialog::on_pushButtonZMedallionControl_clicked()
{
    ZMedallionControlDialog* zMedallionControl = new ZMedallionControlDialog(this);
    zMedallionControl->setModel(walletModel);
    zMedallionControl->exec();
}

void PrivacyDialog::setZMedallionControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMedallionSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMDL()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Medallion Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMDLpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMDL is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MDL </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMDL</b>";
    ...
    // use mints from zMedallion selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMedallionControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMedallionControlDialog::GetSelectedMints();
    }
    ...
    // Clear zmedallion selector in case it was used
    ZMedallionControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMedallion Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Medallion, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMedallion Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMDL </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMDL "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMDL "));
    ui->labelzMDLAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMDLSyncStatus->setVisible(fShow);
}
